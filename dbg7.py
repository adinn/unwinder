...
class FrameConstants(object):
    ...
    def class_init(cls):
        if cls.class_inited:
            return
        cls._interpreter_frame_sender_sp_offset = -1
...
class MethodBCIReader:
    ...
    def class_init(cls):
        ...
        cls.oopDesc_p = gdb.lookup_type("oopDesc").pointer()
        cls.oopDesc_pp = cls.oopDesc_p.pointer()
        cls.method_p = gdb.lookup_type("methodOopDesc").pointer()
        cls.class_inited = True
    ...
    def pc_desc_to_method_bci_stack(self, pc_desc):
        ...
        oops_offset = Types.as_int(nmethod['_oops_offset'])
        oops_base = Types.to_type(base + oops_offset, self.oopDesc_pp)
        # debug_write("oops_base = 0x%x\n" % Types.as_long(oops_base))
        ...
        method_md = (oops_base + (method_idx - 1)).dereference()
...
# n.b in JDK7 this is class methodOopDesc not class Method
class Method(object):
    ...
    def make_name(self, methodptr):
        ...
        klass_oop = constant_pool['_pool_holder']
        klass_oop = klass_oop + 1
        klass_part = Types.to_type(klass_oop, gdb.lookup_type("Klass").pointer())
        klass_sym = klass_part['_name']
        ...
        method_sym_addr = (constant_pool_base + (8 * method_idx)).cast(Types.longp_t).dereference() & 0xfffffffffffffffe
        method_sym = Types.to_type(method_sym_addr, gdb.lookup_type("Symbol").pointer())
        ...
        sig_sym_addr = (constant_pool_base + (8 * sig_idx)).cast(Types.longp_t).dereference() & 0xfffffffffffffffe
        sig_sym = Types.to_type(sig_sym_addr, gdb.lookup_type("Symbol").pointer())
...
class InterpretedMethodInfo(JavaMethodInfo):
    ...
    def __init__(self, entry, bcp):
        ...
        methodptr_slot = methodptr_addr.cast(gdb.lookup_type("methodOopDesc").pointer().pointer())
